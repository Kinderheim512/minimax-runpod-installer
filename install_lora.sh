...
determine_lora_filename() {
  ...
  local cd_header
  cd_header="$(curl -sIL "$url" 2>/dev/null | tr -d '\r' | grep -i '^content-disposition:' | tail -n1)"
  if [[ -n "$cd_header" ]]; then
...
download_lora() {
  ...
  while (( attempt <= DOWNLOAD_MAX_RETRIES )); do
    ...
    if curl -L -C - --fail --retry 3 --retry-delay 5 -o "$dest_file" "$url"; then
      if [[ -s "$dest_file" ]]; then
        return 0
      fi
      log_warn "Fichier téléchargé vide, nouvel essai."
      rm -f "$dest_file"
    else
      log_warn "Échec du téléchargement (tentative ${attempt}/${DOWNLOAD_MAX_RETRIES})."
    fi
...
install_lora() {
  ...
  if ! download_lora "$url" "$dest_file"; then
